...
class SCHarvester(BaseHarvester):
    ...
    def fetch_records(self, start_date, end_date):
        ...
        csv_download = self.requests.get(self.kwargs['csv_url'])

        csv_download.raise_for_status()

        decoded_content = csv_download.content.decode('utf-8')
        ...
        logging.info('Total records harvested %d for date range %s - %s', start_date, end_date, records_harvested)
