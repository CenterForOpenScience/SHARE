...
class AbstractAgent(AbstractShareObject):
    ...
    def resolve_identifiers(self):
        return self.identifiers.exclude(scheme='mailto').filter(same_as__isnull=True)
    ...
    def resolve_related_works(self, offset=None, limit=10):
        ...
        return self.work_relations.filter(same_as__isnull=True)[offset:limit]
    ...
    def resolve_incoming_agent_relations(self, limit=None, offset=None):
        ...
        return self.incoming_agent_relations.filter(same_as__isnull=True)[offset:limit]
    ...
    def resolve_outgoing_agent_relations(self, limit=None, offset=None):
        if limit:
            offset = (offset or 0) + limit
        return self.outgoing_agent_relations.filter(same_as__isnull=True)[offset:limit]
