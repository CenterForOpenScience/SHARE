...
class AbstractCreativeWork(AbstractShareObject):
    ...
    def resolve_identifiers(self):
        return self.identifiers.filter(same_as__isnull=True)

    @graphene.resolve_only_args
    def resolve_tags(self, limit=None, offset=None):
        if limit:
            offset = (offset or 0) + limit
        return self.tags.filter(same_as__isnull=True)[offset:limit]

    @graphene.resolve_only_args
    def resolve_total_related_agents(self):
        return self.agent_relations.filter(same_as__isnull=True).count()

    @graphene.resolve_only_args
    def resolve_related_agents(self, limit=None, offset=None):
        if limit:
            offset = (offset or 0) + limit
        return self.agent_relations.filter(same_as__isnull=True).order_by('order_cited')[offset:limit]

    @graphene.resolve_only_args
    def resolve_total_incoming_work_relations(self):
        return self.incoming_creative_work_relations.filter(same_as__isnull=True).count()

    @graphene.resolve_only_args
    def resolve_incoming_work_relations(self, limit=None, offset=None):
        if limit:
            offset = (offset or 0) + limit
        return self.incoming_creative_work_relations.filter(same_as__isnull=True)[offset:limit]

    @graphene.resolve_only_args
    def resolve_total_outgoing_work_relations(self):
        return self.outgoing_creative_work_relations.filter(same_as__isnull=True).count()

    @graphene.resolve_only_args
    def resolve_outgoing_work_relations(self, limit=None, offset=None):
        if limit:
            offset = (offset or 0) + limit
        return self.outgoing_creative_work_relations.filter(same_as__isnull=True)[offset:limit]
