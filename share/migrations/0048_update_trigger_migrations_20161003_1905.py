...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunSQL(
            sql='CREATE OR REPLACE FUNCTION before_share_creativeworkidentifier_change() RETURNS trigger AS $$\n        DECLARE\n            vid INTEGER;\n        BEGIN\n            INSERT INTO share_creativeworkidentifierversion(persistent_id, action, change_id, creative_work_id, creative_work_version_id, date_created, date_modified, extra_id, extra_version_id, host, same_as_id, same_as_version_id, scheme, uri, uuid) VALUES (NEW.id, TG_OP, NEW.change_id, NEW.creative_work_id, NEW.creative_work_version_id, NEW.date_created, NEW.date_modified, NEW.extra_id, NEW.extra_version_id, NEW.host, NEW.same_as_id, NEW.same_as_version_id, NEW.scheme, NEW.uri, NEW.uuid) RETURNING (id) INTO vid;\n            NEW.version_id = vid;\n            RETURN NEW;\n        END;\n        $$ LANGUAGE plpgsql;',
            reverse_sql='DROP FUNCTION before_share_creativeworkidentifier_change();',
        ),
        migrations.RunSQL(
            sql='DROP TRIGGER IF EXISTS share_creativeworkidentifier_change ON share_creativeworkidentifier;\n\n        CREATE TRIGGER share_creativeworkidentifier_change\n        BEFORE INSERT OR UPDATE ON share_creativeworkidentifier\n        FOR EACH ROW\n        EXECUTE PROCEDURE before_share_creativeworkidentifier_change();',
            reverse_sql='DROP TRIGGER share_creativeworkidentifier_change',
        ),
        migrations.RunSQL(
            sql='CREATE OR REPLACE FUNCTION before_share_personidentifier_change() RETURNS trigger AS $$\n        DECLARE\n            vid INTEGER;\n        BEGIN\n            INSERT INTO share_personidentifierversion(persistent_id, action, change_id, date_created, date_modified, extra_id, extra_version_id, host, person_id, person_version_id, same_as_id, same_as_version_id, scheme, uri, uuid) VALUES (NEW.id, TG_OP, NEW.change_id, NEW.date_created, NEW.date_modified, NEW.extra_id, NEW.extra_version_id, NEW.host, NEW.person_id, NEW.person_version_id, NEW.same_as_id, NEW.same_as_version_id, NEW.scheme, NEW.uri, NEW.uuid) RETURNING (id) INTO vid;\n            NEW.version_id = vid;\n            RETURN NEW;\n        END;\n        $$ LANGUAGE plpgsql;',
