...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunSQL(
            sql='CREATE OR REPLACE FUNCTION before_share_identifier_change() RETURNS trigger AS $$\n        DECLARE\n            vid INTEGER;\n        BEGIN\n            INSERT INTO share_identifierversion(persistent_id, action, change_id, date_created, date_modified, extra_id, extra_version_id, same_as_id, same_as_version_id, url, uuid) VALUES (NEW.id, TG_OP, NEW.change_id, NEW.date_created, NEW.date_modified, NEW.extra_id, NEW.extra_version_id, NEW.same_as_id, NEW.same_as_version_id, NEW.url, NEW.uuid) RETURNING (id) INTO vid;\n            NEW.version_id = vid;\n            RETURN NEW;\n        END;\n        $$ LANGUAGE plpgsql;',
        ...
        migrations.RunSQL(
            sql='CREATE OR REPLACE FUNCTION before_share_relation_change() RETURNS trigger AS $$\n        DECLARE\n            vid INTEGER;\n        BEGIN\n            INSERT INTO share_relationversion(persistent_id, action, change_id, date_created, date_modified, extra_id, extra_version_id, object_work_id, object_work_version_id, relation_type, same_as_id, same_as_version_id, subject_work_id, subject_work_version_id, uuid) VALUES (NEW.id, TG_OP, NEW.change_id, NEW.date_created, NEW.date_modified, NEW.extra_id, NEW.extra_version_id, NEW.object_work_id, NEW.object_work_version_id, NEW.relation_type, NEW.same_as_id, NEW.same_as_version_id, NEW.subject_work_id, NEW.subject_work_version_id, NEW.uuid) RETURNING (id) INTO vid;\n            NEW.version_id = vid;\n            RETURN NEW;\n        END;\n        $$ LANGUAGE plpgsql;',
            reverse_sql='DROP FUNCTION before_share_relation_change();',
        ),
        migrations.RunSQL(
            sql='DROP TRIGGER IF EXISTS share_relation_change ON share_relation;\n\n        CREATE TRIGGER share_relation_change\n        BEFORE INSERT OR UPDATE ON share_relation\n        FOR EACH ROW\n        EXECUTE PROCEDURE before_share_relation_change();',
            reverse_sql='DROP TRIGGER share_relation_change',
        ),
        migrations.RunSQL(
        ...
        migrations.RunSQL(
            sql='CREATE OR REPLACE FUNCTION before_share_workidentifier_change() RETURNS trigger AS $$\n        DECLARE\n            vid INTEGER;\n        BEGIN\n            INSERT INTO share_workidentifierversion(persistent_id, action, change_id, creative_work_id, creative_work_version_id, date_created, date_modified, extra_id, extra_version_id, identifier_id, identifier_version_id, same_as_id, same_as_version_id, uuid) VALUES (NEW.id, TG_OP, NEW.change_id, NEW.creative_work_id, NEW.creative_work_version_id, NEW.date_created, NEW.date_modified, NEW.extra_id, NEW.extra_version_id, NEW.identifier_id, NEW.identifier_version_id, NEW.same_as_id, NEW.same_as_version_id, NEW.uuid) RETURNING (id) INTO vid;\n            NEW.version_id = vid;\n            RETURN NEW;\n        END;\n        $$ LANGUAGE plpgsql;',
        ...
        migrations.RunSQL(
            sql='CREATE OR REPLACE FUNCTION before_share_personidentifier_change() RETURNS trigger AS $$\n        DECLARE\n            vid INTEGER;\n        BEGIN\n            INSERT INTO share_personidentifierversion(persistent_id, action, change_id, date_created, date_modified, extra_id, extra_version_id, identifier_id, identifier_version_id, person_id, person_version_id, same_as_id, same_as_version_id, uuid) VALUES (NEW.id, TG_OP, NEW.change_id, NEW.date_created, NEW.date_modified, NEW.extra_id, NEW.extra_version_id, NEW.identifier_id, NEW.identifier_version_id, NEW.person_id, NEW.person_version_id, NEW.same_as_id, NEW.same_as_version_id, NEW.uuid) RETURNING (id) INTO vid;\n            NEW.version_id = vid;\n            RETURN NEW;\n        END;\n        $$ LANGUAGE plpgsql;',
            reverse_sql='DROP FUNCTION before_share_personidentifier_change();',
        ),
        migrations.RunSQL(
            sql='DROP TRIGGER IF EXISTS share_personidentifier_change ON share_personidentifier;\n\n        CREATE TRIGGER share_personidentifier_change\n        BEFORE INSERT OR UPDATE ON share_personidentifier\n        FOR EACH ROW\n        EXECUTE PROCEDURE before_share_personidentifier_change();',
            reverse_sql='DROP TRIGGER share_personidentifier_change',
        ),
        migrations.RunSQL(
        ...
        migrations.RunSQL(
            sql='CREATE OR REPLACE FUNCTION before_share_contributor_change() RETURNS trigger AS $$\n        DECLARE\n            vid INTEGER;\n        BEGIN\n            INSERT INTO share_contributorversion(persistent_id, action, bibliographic, change_id, cited_name, creative_work_id, creative_work_version_id, date_created, date_modified, extra_id, extra_version_id, order_cited, person_id, person_version_id, same_as_id, same_as_version_id, uuid) VALUES (NEW.id, TG_OP, NEW.bibliographic, NEW.change_id, NEW.cited_name, NEW.creative_work_id, NEW.creative_work_version_id, NEW.date_created, NEW.date_modified, NEW.extra_id, NEW.extra_version_id, NEW.order_cited, NEW.person_id, NEW.person_version_id, NEW.same_as_id, NEW.same_as_version_id, NEW.uuid) RETURNING (id) INTO vid;\n            NEW.version_id = vid;\n            RETURN NEW;\n        END;\n        $$ LANGUAGE plpgsql;',
        ...
        migrations.RunSQL(
            sql='CREATE OR REPLACE FUNCTION before_share_abstractcreativework_change() RETURNS trigger AS $$\n        DECLARE\n            vid INTEGER;\n        BEGIN\n            INSERT INTO share_abstractcreativeworkversion(persistent_id, action, change_id, date_created, date_modified, date_published, date_updated, description, extra_id, extra_version_id, free_to_read_date, free_to_read_type, is_deleted, is_stub, language, rights, same_as_id, same_as_version_id, title, type, uuid) VALUES (NEW.id, TG_OP, NEW.change_id, NEW.date_created, NEW.date_modified, NEW.date_published, NEW.date_updated, NEW.description, NEW.extra_id, NEW.extra_version_id, NEW.free_to_read_date, NEW.free_to_read_type, NEW.is_deleted, NEW.is_stub, NEW.language, NEW.rights, NEW.same_as_id, NEW.same_as_version_id, NEW.title, NEW.type, NEW.uuid) RETURNING (id) INTO vid;\n            NEW.version_id = vid;\n            RETURN NEW;\n        END;\n        $$ LANGUAGE plpgsql;',
